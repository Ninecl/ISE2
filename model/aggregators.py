...
class Graphpooling(Aggregator):
    
    def __init__(self, emb_dim, att_dim, num_heads, num_gcn_layers, out_method):
    ...
    def forward(self, g):
        ...
        self_atts = torch.softmax(torch.bmm(Q_embs, K_embs.permute(0, 2, 1)) / math.sqrt(self.att_dim), dim=-1)
        Z_embs = self.ffn(torch.reshape(torch.bmm(self_atts, V_embs).permute(1, 0, 2), (-1, self.att_dim * self.num_heads)))
        
        if self.out_method == 'sum':
            g_embs = torch.sum(Z_embs, dim=0)
        elif self.out_method == 'mean':
            g_embs = torch.mean(Z_embs, dim=0)
        
        out_embs = torch.cat([g_embs, Z_embs[0], Z_embs[1]])
