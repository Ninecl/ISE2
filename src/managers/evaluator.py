...
from utils.data_utils import move_batch_to_device_dgl, collate_dgl
...
class Evaluator():
    ...
    def eval(self, save=False):
        ...
        neg_scores = []
        neg_labels = []
        dataloader = DataLoader(self.data, batch_size=self.params.batch_size, shuffle=False, num_workers=self.params.num_workers, collate_fn=collate_dgl)

        self.graph_classifier.eval()
        with torch.no_grad():
            for b_idx, batch in enumerate(dataloader):

                graph_data_pos, targets_pos, graph_data_neg, targets_neg = move_batch_to_device_dgl(batch, self.params.device)
                # print([self.data.id2relation[r.item()] for r in data_pos[1]])
                # pdb.set_trace()
                score_pos = self.graph_classifier(graph_data_pos)
                score_neg = self.graph_classifier(graph_data_neg)
