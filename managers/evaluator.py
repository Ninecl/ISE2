import os
import torch

from tqdm import tqdm
from sklearn import metrics
from torch.utils.data import DataLoader

from utils.dgl_utils import collate_dgl, move_batch_to_device_dgl, collate_dgl_CoMPILE


class Evaluator():
    def __init__(self, params, model, data):
        self.params = params
        self.model = model
        self.data = data
        
        if params.model_type == "CoMPILE":
            self.collate_dgl = collate_dgl_CoMPILE
        else:
            self.collate_dgl = collate_dgl

    def eval(self, testing=False):
        pos_scores = []
        pos_labels = []
        neg_scores = []
        neg_labels = []
        dataloader = DataLoader(self.data, batch_size=self.params.eval_batch_size, shuffle=False, num_workers=self.params.num_workers, collate_fn=self.collate_dgl)

        self.model.eval()
        with torch.no_grad():
            pos_scores = torch.empty(0, device=self.params.device)
            neg_scores = torch.empty(0, device=self.params.device)
            pos_labels = torch.empty(0, device=self.params.device)
            neg_labels = torch.empty(0, device=self.params.device)
            all_ranks = torch.empty(0, device=self.params.device)
            for batch in tqdm(dataloader, desc='Evaluating'):
                if self.params.model_type == "CoMPILE":
                    (graphs_pos, r_labels_pos), targets_pos, (graph_neg, r_labels_neg), targets_neg = batch
                    score_pos = self.model(graphs_pos)
                    score_neg = self.model(graph_neg)
                    targets_pos = targets_pos.to(self.params.device)
                    targets_neg = targets_neg.to(self.params.device)
                else:
                    data_pos, targets_pos, data_neg, targets_neg = move_batch_to_device_dgl(batch, self.params.device)
                    score_pos = self.model(data_pos)
                    score_neg = self.model(data_neg)
                
                pos_scores = torch.cat((pos_scores, score_pos.squeeze(1)))
                neg_scores = torch.cat((neg_scores, score_neg.squeeze(1)))
                pos_labels = torch.cat((pos_labels, targets_pos))
                neg_labels = torch.cat((neg_labels, targets_neg))
                
                if testing:
                    scores = torch.cat((score_pos, score_neg.view(score_pos.shape[0], -1)), dim=1)
                    ranks = torch.argsort(torch.argsort(scores, dim=1, descending=True), dim=1)[:, 0] + 1
                    all_ranks = torch.cat((all_ranks, ranks))

        # acc = metrics.accuracy_score(labels, preds)
        labels = torch.cat((pos_labels, neg_labels)).cpu()
        scores = torch.cat((pos_scores, neg_scores)).cpu()
        auc = metrics.roc_auc_score(labels, scores)

        if testing:
            isHit1List = [x for x in all_ranks if x <= 1]
            isHit5List = [x for x in all_ranks if x <= 5]
            isHit10List = [x for x in all_ranks if x <= 10]
            hits_1 = len(isHit1List) / len(all_ranks)
            hits_5 = len(isHit5List) / len(all_ranks)
            hits_10 = len(isHit10List) / len(all_ranks)
            mrr = torch.mean(1 / all_ranks)
            
            return {'auc': auc, 'mrr': mrr, 'hits_1': hits_1, 'hits_5': hits_5, 'hits_10': hits_10}
        
        return {'auc': auc}
